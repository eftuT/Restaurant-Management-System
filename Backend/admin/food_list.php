...
if(isset($_GET['delete'])) {
    ...
    if($delete != "") {
        ...
        echo "<script>alert('Food item deleted successfully!'); window.location.href='food_list.php';</script>";
    }
}
?>
<!doctype html>
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .badge-dinner { background: #cce5ff; color: #004085; }
        .badge-snacks { background: #f8d7da; color: #721c24; }
        .badge-beverage { background: #d1ecf1; color: #0c5460; }
        .badge-dessert { background: #e8daef; color: #6c1b7a; }
        .btn-danger-sm {
            background: #e74c3c;
            color: #fff;
            padding: 5px 12px;
        ...
        .btn-danger-sm:hover { background: #c0392b; transform: scale(1.05); }
...
<div class="main-content">
    <h2 style="margin-bottom:25px;"><i class="fa fa-list"></i> Food List</h2>

    <div class="card">
        <div class="card-header">
            <i class="fa fa-utensils"></i> All Food Items
            <a href="food_add.php" style="float:right;background:#27ae60;color:#fff;padding:5px 15px;border-radius:8px;text-decoration:none;font-size:14px;">
            ...
            <table class="table">
                ...
                <?php $x = $start + 1; while($row = $cat->fetch_assoc()): ?>
                    <tr>
                        ...
                        <td><span class="badge badge-<?php echo $row['food_category']; ?>"><?php echo ucfirst($row['food_category']); ?></span></td>
                        <td><strong><?php echo $row['food_price']; ?> Br</strong></td>
                        <td><?php echo substr($row['food_description'], 0, 40) . (strlen($row['food_description']) > 40 ? '...' : ''); ?></td>
                        <td>
                            <a href="food_list.php?delete=<?php echo $row['id']; ?>" class="btn-danger-sm" onclick="return confirm('Delete this food item?')">
        ...
        <div class="empty-state">
            <i class="fa fa-utensils" style="color:#ddd;"></i>
            <p>No food items added yet. <a href="food_add.php" style="color:#b45f2b;">Add your first food item</a></p>
        </div>
        <?php endif; ?>
    </div>
</div>
